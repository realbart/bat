...
internal static class PathTranslator
{
    ...
    {
        if (fileSystem is not FileSystem fs)
            return hostPath;
    ...
    {
        if (batPath.Length >= 2 && char.IsLetter(batPath[0]) && batPath[1] == ':')
        {
    ...
    private static bool TryGetRootForDrive(IFileSystem fileSystem, char drive, out string root)
    {
        if (fileSystem is FileSystem fs)
            return fs.TryGetNativePath(drive, [], out root);

        try
        {
            root = fileSystem.GetNativePath(drive, []);
            return true;
        }
        catch
        {
